...
The lifecycle (`Todo → In Progress → Done`) is implemented by the `/work` orchestrator together with Linear's GitHub integration. `/work` runs `tools/linear pickup` on dispatch; the Linear↔GitHub integration moves an issue to In Progress when a branch/PR references it and to Done when that PR merges (via `Fixes PA-<id>` in the PR body), so hand-merged PRs close their issue too. Non-dev issues closed by a proposal move to Done when the applier applies the closing proposal. Setup: [DEV_WORKFLOW.md](DEV_WORKFLOW.md).
